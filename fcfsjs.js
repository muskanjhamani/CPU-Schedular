...
document.getElementById('calculate').addEventListener('click',function(){
    ...
    for(let i = 0; i < arrivalArr.length; i++) {
        corres[i].start_time = (i == 0)?corres[i].arrival:Math.max(corres[i-1].completion_time,corres[i].arrival);
        ...
        total_idle_time += (i == 0)?(corres[i].arrival):(corres[i].start_time - corres[i-1].completion_time);
    }
    ...
    cpu_utilisation = ((corres[arrivalArr.length -1].completion_time - total_idle_time) / ( corres[arrivalArr.length -1].completion_time))*100;
    throughput = arrivalArr.length / (corres[arrivalArr.length-1].completion_time - corres[0].arrival);
   ...
   for(let i=0;i<corres.length;i++) {
    if (startGantt<Number(corres[i].arrival)) { //condition when cpu is idle 
         ganttChartData.push([
     ...
     } if(startGantt>=Number(corres[i].arrival)){ //process 
         ganttChartData.push([
             "Time",
             "P" + corres[i].id,
             "",
 ...
 let gantt_chart=document.createElement('div');
    ...
    for(let i=0;i<corres.length;i++) {
     if (startTimeline<Number(corres[i].arrival)) { //condition when cpu is idle 
          startTimeline=Number(corres[i].arrival);
      } 
    if(startTimeline>=Number(corres[i].arrival)){ //process 
          timeChartData.push([
              `P${corres[i].id}`,
              "P" + corres[i].id,
              "",
  ...
  timeChartData.sort((a, b) => parseInt(a[0].substring(1, a[0].length)) - parseInt(b[0].substring(1, b[0].length)));
