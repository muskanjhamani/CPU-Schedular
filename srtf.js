...
for(let i = 0; i < arrivalArr.length; i++) {
    ...
    while(completed != arrivalArr.length) {
        ...
        if(idx != -1) {
            ...
            if(burst_remaining[idx] == 0) {
                ...
                corres[idx].turnAroundTime = Number(corres[idx].completion_time) - Number(corres[idx].arrival);
                corres[idx].waitingTime = Number(corres[idx].turnAroundTime) - Number(corres[idx].burst);
                corres[idx].response_time = Number(corres[idx].start_time) - Number(corres[idx].arrival);
    ...
    for(let i = 0; i < arrivalArr.length; i++) {
        ...
        max_completion_time = Math.max(max_completion_time,Number(corres[i].completion_time));
    }
    ...
    cpu_utilisation = ((max_completion_time - total_idle_time) / max_completion_time )*100;
