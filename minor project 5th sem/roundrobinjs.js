...
//     }    
    ...
    while(completed != arrivalArr.length) {
        ...
        if(burst_remaining[idx] == corres[idx].burst_time) {
            corres[idx].start_time = Math.max(current_time,Number(corres[idx].arrival_time));
        ...
        if(burst_remaining[idx]-Number(tq) > 0) {
            ...
            if (startGantt >= Number(corres[idx].start_time)) {
                ganttChartData.push([Number(corres[idx].id), "P" + Number(corres[idx].id), startGantt, current_time]);
            ...
            corres[idx].turnaround_time = Number(corres[idx].completion_time) - Number(corres[idx].arrival_time);
            corres[idx].waiting_time = Number(corres[idx].turnaround_time) - Number(corres[idx].burst_time);
            corres[idx].response_time = Number(corres[idx].start_time) - Number(corres[idx].arrival_time);
        ...
        for(let  i = 1; i < arrivalArr.length; i++) {
            if(burst_remaining[i] > 0 && corres[i].arrival_time <= current_time && mark[i] == 0) {
                q.enqueue(i);
                mark[i] = 1;
            }
        }
    ...
    cpu_utilisation = ((Number(corres[arrivalArr.length-1].completion_time) - total_idle_time) / Number(corres[arrivalArr.length-1].completion_time))*100;
    throughput = arrivalArr.length / Number((corres[arrivalArr.length-1].completion_time) - Number(corres[0].arrival_time));
...
let TimelineHeading=document.createElement('h3');
  ...
  timeChartARR.sort((a, b) => parseInt(a[0].substring(1, a[0].length)) - parseInt(b[0].substring(1, b[0].length)));
